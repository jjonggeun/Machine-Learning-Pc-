...
def k_means_clustering(data, K):
    ...
    while True:
        ...
        for point in data:
            # 각 중심과의 거리를 계산합니다.
            distances = [l1_norm(point.reshape(1, -1), u[j].reshape(1, -1)) for j in range(K)]
...
# 데이터 불러오기
file_path = "C:\\Users\\pc\\Desktop\\3학년\\1학기\\머러실\\Clustering_data.csv"
    ...
    for k in range(1, 11):
        ...
        for cluster in clusters:
            if len(cluster) > 0:
                ...
                # 각 포인트와 중심 사이의 거리를 계산합니다.
                variance = np.mean([l1_norm(point.reshape(1, -1), cluster_center.reshape(1, -1)) for point in cluster])
